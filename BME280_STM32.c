...
#define BME280_DELAY_MS(ms) vTaskDelay(pdMS_TO_TICKS(ms))
...
HAL_StatusTypeDef BME280_I2C_Read(uint16_t DevAddress, uint8_t RegAddress, uint8_t *pData, uint16_t Size) {
	HAL_StatusTypeDef status;
	if(osMutexAcquire(i2cMutexHandle, 1000) == osOK){
        status = HAL_I2C_Mem_Read(&hi2c1, DevAddress, RegAddress, 1, pData, Size, 1000);
...
HAL_StatusTypeDef BME280_I2C_Write(uint16_t DevAddress, uint8_t RegAddress, uint8_t *pData, uint16_t Size) {
	HAL_StatusTypeDef status;
	if(osMutexAcquire(i2cMutexHandle, 1000) == osOK){
        status = HAL_I2C_Mem_Write(&hi2c1, DevAddress, RegAddress, 1, pData, Size, 1000);
    	osMutexRelease(i2cMutexHandle);
    }else{
    	status = HAL_ERROR;
    }
...
void TrimRead(void)
{
	...
	BME280_I2C_Read(BME280_ADDRESS, 0x88, trimdata, 25);
	// Read NVM from 0xE1 to 0xE7
	BME280_I2C_Read(BME280_ADDRESS, 0xE1, (uint8_t *)trimdata+25, 7);
...
int BME280_Config (uint8_t osrs_t, uint8_t osrs_p, uint8_t osrs_h, uint8_t mode, uint8_t t_sb, uint8_t filter)
{
	...
	if (BME280_I2C_Write(BME280_ADDRESS, RESET_REG, &datatowrite, 1) != osOK)
	{
		return -1;
	}
	...
	datatowrite = osrs_h;
	if (BME280_I2C_Write(BME280_ADDRESS, CTRL_HUM_REG, &datatowrite, 1) != osOK)
	{
		return -1;
	}
	BME280_DELAY_MS(100);
	BME280_I2C_Read(BME280_ADDRESS, CTRL_HUM_REG, &datacheck, 1);
	...
	if (BME280_I2C_Write(BME280_ADDRESS, CONFIG_REG, &datatowrite, 1) != osOK)
	{
		return -1;
	}
	BME280_DELAY_MS(100);
	BME280_I2C_Read(BME280_ADDRESS, CONFIG_REG, &datacheck, 1);
	if (datacheck != datatowrite)
	{
		return -1;
	}
	...
	if (BME280_I2C_Write(BME280_ADDRESS,CTRL_MEAS_REG, &datatowrite, 1) != osOK)
	{
		return -1;
	}
	BME280_DELAY_MS(100);
	BME280_I2C_Read(BME280_ADDRESS, CTRL_MEAS_REG, &datacheck, 1);
	if (datacheck != datatowrite)
	{
		return -1;
	}
...
int BMEReadRaw(void)
{
	...
	BME280_I2C_Read(BME280_ADDRESS, ID_REG, &chipID, 1);

	if (chipID == 0x60)
	{
		// Read the Registers 0xF7 to 0xFE
		BME280_I2C_Read(BME280_ADDRESS, PRESS_MSB_REG, RawData, 8);
...
void BME280_WakeUP(void)
{
	...
	BME280_I2C_Read(BME280_ADDRESS, CTRL_MEAS_REG, &datatowrite, 1);

	// modify the data with the forced mode
	datatowrite = datatowrite | MODE_FORCED;

	// write the new data to the register
	BME280_I2C_Write(BME280_ADDRESS,CTRL_MEAS_REG, &datatowrite, 1);
	BME280_DELAY_MS(100);
}
